...
const errMsg = {
  timeout: 'Login timeout: Unknown error',
...
class QzoneApi {
  static login(uin, pwd) {
    return new Promise(async (resolve, reject) => {
      ...
      setTimeout(() => reject(new Error(QzoneApi.errMsg.timeout)), 5000)
      try {
        // 监听页面加载完成事件
        // 当模拟登录成功后会执行跳转动作
        // 在此处获取 cookie 和用户信息
        ...
        await page.on('onResourceReceived', async function(response) {
          ...
          if (url.startsWith(captchaUrl)) {
            // 异地登录，需要验证码
            reject(new Error(QzoneApi.errMsg.check))
          } else if (
            status === 200 &&
            stage === 'start' &&
            url.startsWith(api.login)
          ) {
            // 获取 Set-Cookie 相应头中是否包含 skey 字符串
            // 如果包含则表示登录成功
            ...
            if (!hasKey) {
              await instance.exit()
              reject(new Error(QzoneApi.errMsg.failed))
            }
          }
        })
...
QzoneApi.errMsg = errMsg
module.exports = QzoneApi
